...
dir_img = '/content/drive/My Drive/ML /img2/ap/'
dir_mask = '/content/drive/My Drive/ML /img2/ver/'
...
def train_net(net,
              ...
              img_scale=0.5):
    momentum = 0.9
    lr = 0.04*(1-momentum) #for RMSProp
    weight_decay = 1e-10
    lr_decay = 0.001
    ...
    optimizer = optim.RMSprop(net.parameters(), lr=lr, weight_decay=weight_decay,momentum=momentum,centered = True)
    ...
    for epoch in range(20):
        ...
        with tqdm(total=n_train, desc=f'Epoch {epoch + 1}/{epochs}', unit='img') as pbar:
            for batch in train_loader:
                ...
                # if (epoch == 2):
                #     image = pred[0,0,:,:].cpu().data.numpy()
                #     df = pd.DataFrame(image) 
                #     df.to_csv('/content/images1/pred'+str(i) +'.csv') 
                #     i+=1
                ...
                global_step += 1
                if global_step % (len(dataset) // (2)) == 0:
                    val_score = 0
